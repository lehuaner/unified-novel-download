...
# 文件名：installer.sh
# 功能：
#   1. 自动通过 GitHub API 获取 unified-novel-download 最新版本
...
GITHUB_API_URL="https://api.github.com/repos/lehuaner/unified-novel-download/releases/latest"
...
case "$PLATFORM" in
    Linux)
        if $IS_TERMUX; then
            ...
            BINARY_NAME="UnifiedNovelDownloader-Android_${ANDROID_ARCH}-v${VERSION}"
            log_info "检测到 Termux（架构：${ANDROID_ARCH}），将使用 Android 原生版本。"
        else
            case "$ARCH" in
                x86_64|amd64)
                    if $IS_MUSL; then
                        BINARY_NAME="UnifiedNovelDownloader-Linux_musl_amd64-v${VERSION}"
                    else
                        BINARY_NAME="UnifiedNovelDownloader-Linux_amd64-v${VERSION}"
                    fi
                    ;;
                aarch64|arm64)
                    if $IS_MUSL; then
                        BINARY_NAME="UnifiedNovelDownloader-Linux_musl_arm64-v${VERSION}"
                    else
                        BINARY_NAME="UnifiedNovelDownloader-Linux_arm64-v${VERSION}"
                    fi
                    ;;
                *)
    ...
    Darwin)
        case "$ARCH" in
            arm64)
                BINARY_NAME="UnifiedNovelDownloader-macOS_arm64-v${VERSION}"
                ;;
            x86_64)
                BINARY_NAME="UnifiedNovelDownloader-macOS_amd64-v${VERSION}"
                ;;
            *)
...
DOWNLOAD_URL="https://github.com/lehuaner/unified-novel-download/releases/download/${TAG_NAME}/${BINARY_NAME}"
