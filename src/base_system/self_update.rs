...
const OWNER: &str = "lehuaner";
const REPO: &str = "unified-novel-download";
...
fn get_accelerated_url(original_url: &str) -> String {
    // 直接使用 GitHub Releases 下载地址（不依赖第三方 CDN）。
...
///
/// `run.sh` 中的 `exec` 行格式如：
///   exec "${SCRIPT_DIR}/UnifiedNovelDownloader-Android_arm64-v2.3.2" --server "$@"
/// 更新后需变为：
///   exec "${SCRIPT_DIR}/UnifiedNovelDownloader-Android_arm64" --server "$@"
fn patch_run_sh_if_exists(new_exe: &Path) -> Result<()> {
    ...
    let platform_key = detect_platform_keyword()?;
    let versioned_prefix = format!("UnifiedNovelDownloader-{platform_key}-v");
    let canonical = format!("UnifiedNovelDownloader-{platform_key}");
    ...
    for line in content.lines() {
        if let Some(idx) = line.find(&versioned_prefix) {
            // 将 "UnifiedNovelDownloader-<platform>-v<version>" 替换为规范名
            let before = &line[..idx];
...
fn canonical_executable_name() -> Result<OsString> {
    ...
    // 例如：
    // - Linux (glibc): UnifiedNovelDownloader-Linux_amd64 / UnifiedNovelDownloader-Linux_arm64
    // - Linux (musl):  UnifiedNovelDownloader-Linux_musl_amd64 / UnifiedNovelDownloader-Linux_musl_arm64
    // - Android:       UnifiedNovelDownloader-Android_arm64 / UnifiedNovelDownloader-Android_arm32
    // - Windows:       UnifiedNovelDownloader-Win64.exe / UnifiedNovelDownloader-WinArm64.exe
    // - macOS:         UnifiedNovelDownloader-macOS_arm64 / UnifiedNovelDownloader-macOS_amd64
    let platform_key = detect_platform_keyword()?;
    let mut name = format!("UnifiedNovelDownloader-{platform_key}");
    if cfg!(windows) {
        name.push_str(".exe");
    }
