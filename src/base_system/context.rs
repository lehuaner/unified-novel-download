...
pub struct Config {
    ...
    ///
    /// 非空时启用"签名-only sidecar"模式：Rust 构建 URL+headers，
    /// 调 unidbg 生成签名头，再直连番茄 API 并解密。
    /// 留空则走第三方 API 端点列表（`api_endpoints`）。
    #[serde(default)]
    pub unidbg_signer_url: String,
...
impl ConfigSpec for Config {
    ...
    fn fields() -> &'static [FieldMeta] {
        static FIELDS: [FieldMeta; 45] = [
            ...
            FieldMeta {
                name: "unidbg_signer_url",
                description: "unidbg签名sidecar地址(留空走第三方API)",
            },
            FieldMeta {
