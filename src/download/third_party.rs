...
/// unidbg 签名 sidecar 模式：用 FqApiClient 直连番茄 API。
///
/// `client` 是预先创建的 `FqApiClient`（线程安全，可共享）。
/// `book_id` 用于 batch_full 请求。
pub(crate) fn fetch_group_unidbg(
    client: &FqApiClient,
    book_id: &str,
    group: &[ChapterRef],
) -> Result<serde_json::Value> {
    let ids = group
        ...
        .collect::<Vec<_>>()
        .join(",");
    client.get_contents(&ids, book_id)
}
