...
impl FanqieWebNetwork {
    ...
    pub(crate) fn fetch_chapter_list(&self, book_id: &str) -> Option<Vec<Value>> {
        ...
        debug!("API 重试仍失败：{:?}，尝试 HTML 页面回退", last_error);

        // HTML 页面回退：当 API 返回空数据时，从页面 __INITIAL_STATE__ 提取章节列表
        if let Some(list) = self.fetch_chapter_list_from_html(book_id) {
            info!("通过 HTML 页面回退成功获取章节列表 (book_id={book_id})");
            return Some(list);
        }
    ...
    /// HTML 页面回退：从 `https://fanqienovel.com/page/{book_id}` 的 `__INITIAL_STATE__` 提取章节列表。
    ///
    /// 当目录 API 返回空数据（反爬/风控）时，页面 HTML 中仍嵌有完整的 `__INITIAL_STATE__` JSON，
    /// 其中 `page.chapterListWithVolume` / `page.chapterList` 包含章节信息。
    fn fetch_chapter_list_from_html(&self, book_id: &str) -> Option<Vec<Value>> {
        let page_url = format!("https://fanqienovel.com/page/{book_id}");
        debug!("HTML 回退：请求页面 {page_url}");
        ...
        if resp.status().as_u16() == 404 {
            warn!("HTML 回退：书籍 ID {book_id} 不存在 (404)");
            return None;
        }
        ...
        let html = match resp.text() {
            Ok(t) => t,
            Err(e) => {
                warn!("HTML 回退读取 body 失败: {e}");
                return None;
            }
        };

        // 从 HTML 中提取 __INITIAL_STATE__ JSON
        let json_text = extract_initial_state_json(&html)?;
        let value: Value = serde_json::from_str(&json_text).ok()?;

        // __INITIAL_STATE__.page 中包含 chapterListWithVolume / chapterList
        let page = value.get("page").unwrap_or(&value);
        let list = Self::parse_chapter_data(page)?;

        if list.is_empty() {
            warn!("HTML 回退：__INITIAL_STATE__ 中章节数组为空");
            return None;
        }

        debug!("HTML 回退成功：{} 章", list.len());
        Some(list)
    }
